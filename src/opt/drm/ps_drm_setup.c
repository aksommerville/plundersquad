...
int drm_open_file() {
  ...
  if (ioctl(ps_drm.fd,DRM_IOCTL_VERSION,&version)<0) {
...
static int drm_select_best_mode(drmModeResPtr res) {
  //fprintf(stderr,"%s...\n",__func__);
  //fprintf(stderr,"Client requested %dx%d@%d\n",ps_drm.delegate.fbw,ps_drm.delegate.fbh,ps_drm.delegate.rate);
  ...
  for (;conni<res->count_connectors;conni++) {
    drmModeConnectorPtr conn=drmModeGetConnector(ps_drm.fd,res->connectors[conni]);
    if (!conn) continue;
  ...
  ps_drm.mmw=bestconn->mmWidth;
  ps_drm.mmh=bestconn->mmHeight;
  ps_drm.connid=bestconn->connector_id;
  ps_drm.encid=bestconn->encoder_id;
  ps_drm.w=bestmode.hdisplay;
  ps_drm.h=bestmode.vdisplay;
  ps_drm.rate=bestmode.vrefresh;
  memcpy(&ps_drm.mode,&bestmode,sizeof(drmModeModeInfo));
  drmModeFreeConnector(bestconn);
  return 0;
}
...
int drm_configure() {
  ...
  drmModeResPtr res=drmModeGetResources(ps_drm.fd);
  if (!res) {
  ...
  drmModeEncoderPtr encoder=drmModeGetEncoder(ps_drm.fd,ps_drm.encid);
  if (!encoder) {
    fprintf(stderr,"drmModeGetEncoder() failed\n");
    drmModeFreeResources(res);
    return -1;
  }
  ps_drm.crtcid=encoder->crtc_id;
  drmModeFreeEncoder(encoder);
  
  // Store the current CRTC so we can restore it at quit.
  if (!(ps_drm.crtc_restore=drmModeGetCrtc(ps_drm.fd,ps_drm.crtcid))) {
...
int drm_init_gx() {
  ...
  if (!(ps_drm.gbmdevice=gbm_create_device(ps_drm.fd))) return -1;
  
  if (!(ps_drm.gbmsurface=gbm_surface_create(
    ps_drm.gbmdevice,
    ps_drm.w,ps_drm.h,
    GBM_FORMAT_XRGB8888,
  ...
  if (!(ps_drm.egldisplay=eglGetPlatformDisplayEXT(
    EGL_PLATFORM_GBM_KHR,ps_drm.gbmdevice,0
  ))) return -1;
  
  EGLint major=0,minor=0;
  if (!eglInitialize(ps_drm.egldisplay,&major,&minor)) return -1;
  ...
  EGLint n;
  if (!eglChooseConfig(
    ps_drm.egldisplay,config_attribs,&eglconfig,1,&n
  )||(n!=1)) return -1;
  
  if (!(ps_drm.eglcontext=eglCreateContext(
    ps_drm.egldisplay,eglconfig,EGL_NO_CONTEXT,context_attribs
  ))) return -1;
  
  if (!(ps_drm.eglsurface=eglCreateWindowSurface(
    ps_drm.egldisplay,eglconfig,ps_drm.gbmsurface,0
  ))) return -1;
  
  eglMakeCurrent(ps_drm.egldisplay,ps_drm.eglsurface,ps_drm.eglsurface,ps_drm.eglcontext);
  
  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
  
  if (ps_video_resized(ps_drm.w,ps_drm.h)<0) return -1;
